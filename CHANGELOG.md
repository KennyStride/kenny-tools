...
### Added
- **One-command GitHub Release + Homebrew Cask publisher**: Added `scripts/release/publish_github_release_and_cask.cjs` and `npm run release:github-and-cask` to build a `universal.dmg`, upload assets to GitHub Release, and update `Casks/kenny-tools.rb` (with `--skip-build` / `--skip-gh` / `--skip-cask` / `--dry-run` support).
...
- **Release workflow restores automatic Homebrew Cask updates**: `release.yml` now restores the `update-homebrew-cask` job to compute `sha256` from the published `*_universal.dmg`, update `Casks/kenny-tools.rb`, and open a cask PR after release assets are available.
