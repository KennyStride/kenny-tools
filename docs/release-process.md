...
推荐一键脚本（会执行 `universal.dmg` 构建、上传 GitHub Release 资产、更新 `Casks/kenny-tools.rb`）：
...
1. 先上传 GitHub Release 资产，再推送 `Casks/kenny-tools.rb` 更新，避免 cask 链接短暂 404。
2. `Casks/kenny-tools.rb` 中的 `version`、`sha256` 必须与 Release 中实际 `*_universal.dmg` 一致。
